...
<section class="content">
	...
	<div class="row bgwhite margin">
		<ul class="breadcrumb">
								...
								<li class="logofont active">Forum</li>
      </ul>
		
		
		<center><h3 class="logofont">Forum</h3></center>
                              ...
                              <div  class="show">
<?php
	//$qry1= mysql_query("select UserName from employeemaster where UserName=".$_GET['Uname']."");
    $qry = mysql_query("select employeeid from employeemaster where UserName='".$_SESSION['Uname']."' ");
   $rw1 = mysql_fetch_row($qry);
   $employeeid = $rw1[0];
   
  // $qry2=mysql_query("select q.question,q.imgurl,q.dateofquestion,q.viewedcount,p.submitid from queruforumquestion q inner join projectstatusdetail p on p.submitid=q.submitid where p.employeeid=$employeeid");
   
   
   $qry2=mysql_query("select q.questionid,p.submitid,q.question,q.imgurl,q.dateofquestion,q.ViewedCount,m.moduleid,p.employeeid,m.modulename from queruforumquestion q inner join projectstatusdetail p on p.submitid=q.submitid inner join modulemaster m on m.moduleid=p.moduleid where p.employeeid=$employeeid");
   
   
  echo "<center>";
  echo "<div class='container'>";
  echo "<div class='row'>";
	// echo "<div class='col-lg-2'>Question</div>";
	  ...
	  {
	  $i=1;
	     while($rw=mysql_fetch_assoc($qry2))
		 {
		 extract($rw);
		
		// $qry2=mysql_query("select ProjectTitle from projectmaster where ProjectId=$ProjectId");	
		 //$rw2 = mysql_fetch_row($qry2);	
		  
	// echo "<tr><td>$question</td><td>$imgurl</td><td>$dateofquestion</td> </tr>";
		
		if($i % 2 == 0)
		{
		 echo "<div class='row sky rw'>";
}else
{
 echo "<div class='row rw bg-blue'>";
}
		 echo "<div class='col-lg-2'><img src='../../img/login.png' height='50px' width='50px' /></div>
				<div class='col-lg-2'> Question :- $question</div>
				<div class='col-lg-2'>  Module :- $modulename</div>
				<div class='col-lg-2'><a href='forumans.php?id=$questionid'>FORUM ANSWER</a></div></div>";
		 $i++;
		 }
