...
<?php
		if(isset($_GET["projectid"]) && $_GET['flg']=="Approve")
	{
		$qry=mysql_query("update projectmaster set Status='1' where  ProjectId=".$_GET['projectid']."");
	    if(!$qry)
	   {
	   $msg = mysql_error();
	   }
	...
	if(isset($_GET["projectid"]) && $_GET['flg']=="Rejected")
	{
		$qry=mysql_query("update projectmaster set Status='0' where  ProjectId=".$_GET['projectid']."");
	    if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
<section class="content">
	...
	<div class="row bgwhite margin">
		...
		<center><h3 class="logofont">Project more Details</h3></center>
...
<?php 
									  if(isset($_GET['id'])){
    $qry1 =mysql_query("select * from projectmaster where ProjectId=".$_GET['id']."");
	...
	// $//qry= mysql_query("select * from projectmaster");
	 ...
	 echo "<center>";
	  ...
	  if($qry1)
	  {
	     while($rw=mysql_fetch_assoc($qry1))
		 {
		 extract($rw);
		  ...
		  echo "<tr> </tr>
			  ...
			  <tr> <td> ProjectTitle </td> <td>$ProjectTitle</td> </tr>
			   <tr> <td> LogoURL </td> <td>$LogoURL</td> </tr>
			    <tr> <td> ClientId </td> <td>".$rw3[0]."</td> </tr>
				 <tr> <td> LanguageId </td> <td>".$rw4[0]."</td> </tr>
				  <tr> <td> ProjectDetail </td> <td>$ProjectDetail</td> </tr>
				   <tr> <td> StartDate </td> <td>$StartDate</td> </tr>
					...
					<tr><td> Status </td> <td>$Status   
							...
							<a href=?projectid=$ProjectId&flg=Approve class=btn>  Approve</a>
							<a href=?projectid=$ProjectId&flg=Rejected class=btn>  Rejected</a>
							</td>
					 </tr>
