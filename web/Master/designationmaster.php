...
<!-- InstanceBeginEditable name="head" -->
		...
		<?php
 // $cn = new PDO('mysql:host=localhost;dbname=queryforum', "root", '');
 
	$msg="";
	$DesignationId=$DesignationName="";
	
	 $qry= mysql_query("select max(DesignationId)+1 from designationmaster");
								   $designationid=mysql_fetch_row($qry);
	...
	if(isset($_POST['insert']))
	{
	   ...
	   $qry =mysql_query("insert into designationmaster values($designationid,'$designationname');");
	   if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
///edit btn code
	if(isset($_GET["designationid"]) && $_GET['flg']=="Edit")
	{
	    $qry = mysql_query("select DesignationId,DesignationName from designationmaster where designationid=".$_GET['designationid']."");
	...
	if(isset($_POST['update']))
	{
		$designationid= $_POST['designationid'];
	   $designationname=$_POST['designationname'];
	   $qry =mysql_query("update designationmaster set designationname='$designationname' where designationid=$designationid");
	if(!$qry)
	   {
	   $msg = mysql_error();
	   }
	   ...
	   {
	   $msg = "Record Updated...";
	   header('location: designationmaster.php');
	   }
	}
	//delete code
	if(isset($_GET["designationid"]) && $_GET['flg']=="Delete")
	{
	    $qry = mysql_query("delete from designationmaster where designationid=".$_GET['designationid']."");
	   if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
<!-- InstanceBeginEditable name="body" -->
	...
	<div class="row bgwhite margin">
			<ul class="breadcrumb">
								...
								<li class="logofont active">Designation Master</li>
      </ul>
		
		
		
		<center><h3 class="logofont">Designation Master</h3></center>
	...
	<div class="col-xs-offset-2 col-lg-8">
		<div class="row bg">
			...
			<div class="form-group">
                           ...
                           <div class="row">
                           <div class="col-lg-4">
                          	 <label class="black">Enter DesignationId</label>
                           </div>
                            <div class="col-lg-5">
    <input type="text" name="designationid" class="form-control" value="<?php if(isset($DesignationId)) { echo $DesignationId; }?>" />
                            </div>
                            </div>
                            </div>
                                   <div class="form-group">
                           ...
                           <div class="row">
                           <div class="col-lg-4">
                          	 <label class="black">Enter DesignationName</label>
                           </div>
                            <div class="col-lg-5">
 <input type="text" name="designationname" class="form-control" value="<?php if(isset($DesignationName)) { echo $DesignationName; }?> "/>
                            </div>
                            </div>
                            </div>
                            		 <div class="form-group">
                        ...
                        <div  class="show">
<?php
    $qry =mysql_query("select DesignationId,DesignationName from designationmaster");
	 echo "<center>";
	  echo "<table>";
	  echo "<tr><th> DesignationId </th><th> Designation Name </th><th></th></tr>";
	  if($qry)
	  {
	     while($rw=mysql_fetch_assoc($qry))
		 {
		 extract($rw);
		 echo "<tr><td>$DesignationId</td><td>$DesignationName</td>
		 
		 
		 <td>
		  <button class='btn btn-default btn-xs'>
		<a href=?designationid=$DesignationId&flg=Edit> <i class='fa fa-pencil'></a></i></button>
	
		  <button class='btn btn-default btn-xs'>
		 <a href=?designationid=$DesignationId&flg=Delete><i class='fa fa-times'></a></i></button></td></tr>";
