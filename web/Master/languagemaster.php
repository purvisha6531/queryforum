...
<?php
 // $cn = new PDO('mysql:host=localhost;dbname=queryforum', "root", '');
	...
	$LanguageId=$LanguageName="";
	 $qry= mysql_query("select max(LanguageId)+1 from languagemaster");
								   $languageid=mysql_fetch_row($qry);
	...
	if(isset($_POST['insert']))
	{
	   ...
	   $qry =mysql_query("insert into languagemaster values($languageid,'$languagename');");
	   if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
///edit btn code
	if(isset($_GET["languageid"]) && $_GET['flg']=="Edit")
	{
	    $qry = mysql_query("select LanguageId,LanguageName from languagemaster where languageid=".$_GET['languageid']."");
	...
	if(isset($_POST['update']))
	{
		$languageid= $_POST['languageid'];
		$languagename=$_POST['languagename'];
	   $qry =mysql_query("update languagemaster set languagename='$languagename' where languageid=$languageid");
	if(!$qry)
	   {
	   $msg = mysql_error();
	   }
	   ...
	   {
	   $msg = "Record Updated...";
	   header('location: languagemaster.php');
	   }
	}
	//delete code
	if(isset($_GET["languageid"]) && $_GET['flg']=="Delete")
	{
	    $qry = mysql_query("delete from languagemaster where languageid=".$_GET['languageid']."");
	   if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
<!-- InstanceBeginEditable name="body" -->
	...
	<div class="row bgwhite margin">
			...
			<ul class="breadcrumb">
								...
								<li class="logofont active">Language Master</li>
      </ul>
		
		
		
		
		<center><h3 class="logofont">Language Master</h3></center>
	...
	<div class="col-xs-offset-2 col-lg-8">
		<div class="row bg">
			...
			<div class="form-group">
                           ...
                           <div class="row">
                           <div class="col-lg-4">
                          	 <label class="black">Enter LanguageId</label>
                           </div>
                            <div class="col-lg-5">
    <input type="text" name="languageid" class="form-control" value="<?php if(isset($LanguageId)) { echo $LanguageId; }?>" />
                            </div>
                            </div>
                            </div>
                                   <div class="form-group">
                           ...
                           <div class="row">
                           <div class="col-lg-4">
                          	 <label class="black">Enter LanguageName</label>
                           </div>
                            <div class="col-lg-5">
 <input type="text" name="languagename" class="form-control" value="<?php if(isset($LanguageName)) { echo $LanguageName; }?> "/>
                            </div>
                            </div>
                            </div>
                            		 <div class="form-group">
                        ...
                        <div  class="show">
<?php
    $qry =mysql_query("select LanguageId,LanguageName from languagemaster");
	 echo "<center>";
	  echo "<table>";
	  echo "<tr><th> LanguageId </th><th> Language Name </th><th></th><th></th></tr>";
	  if($qry)
	  {
	     while($rw=mysql_fetch_assoc($qry))
		 {
		 extract($rw);
		 echo "<tr><td>$LanguageId</td><td>$LanguageName</td>
		 <td>
		  <button class='btn btn-default btn-xs'>
		<a href=?languageid=$LanguageId&flg=Edit> <i class='fa fa-pencil'></a></i></button>
	
		  <button class='btn btn-default btn-xs'>
		 <a href=?languageid=$LanguageId&flg=Delete> <i class='fa fa-times'></a></i></button></td></tr>";
		 }
	  }
      echo "</table>";
	  echo "</center>";
