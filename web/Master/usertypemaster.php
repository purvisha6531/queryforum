...
<?php
 // $cn = new PDO('mysql:host=localhost;dbname=queryforum', "root", '');
	...
	$UserTypeId=$UserTypeName="";
								   ...
								   $qry= mysql_query("select max(UserTypeId)+1 from usertypemaster");
								   $userid=mysql_fetch_row($qry);
								   $UserTypeId=$userid[0];
	...
	if(isset($_POST['insert']))
	{
	   ...
	   $username=$_POST['usertypename'];
	   $qry =mysql_query("insert into usertypemaster values($userid,'$username');");
	   if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
///edit btn code
	if(isset($_GET["usertypeid"]) && $_GET['flg']=="Edit")
	{
	    $qry = mysql_query("select UserTypeId,UserTypeName from usertypemaster where usertypeid=".$_GET['usertypeid']."");
	...
	$userid= $_POST['usertypeid'];
	   $username=$_POST['usertypename'];
	   $qry =mysql_query("update usertypemaster set usertypename='$username' where usertypeid=$userid");
	if(!$qry)
	   {
	   $msg = mysql_error();
	   }
	   ...
	   {
	   $msg = "Record Updated...";
	   header('location: usertypemaster.php');
	   }
	}
	//delete code
	if(isset($_GET["usertypeid"]) && $_GET['flg']=="Delete")
	{
	    $qry = mysql_query("delete from usertypemaster where usertypeid=".$_GET['usertypeid']."");
	   if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
<!-- InstanceBeginEditable name="body" -->
	...
	<div class="row bgwhite margin">
		<ul class="breadcrumb">
								...
								<li class="logofont active">User Type Master</h4></li>
      </ul>
		
		<center><h3 class="logofont">User Type Master</h3></center>
			...
			<div class="form-group">
				 ...
				 <form method="post" action="">
					 <div class="form-group">
                           <div class="row">
                           <div class="col-lg-4">
                          	 <label class="black">Enter UserTypeId</label>
                           </div>
                            <div class="col-lg-5">
     <input type="text" name="usertypeid" class="form-control" value="<?php if(isset($UserTypeId)) { echo $UserTypeId; }?>" />
                            </div>
                       </div>
                   </div>
                           ...
                           <div class="row">
                           <div class="col-lg-4">
                          	 <label class="black">Enter UserTypeName</label>
                           </div>
                            <div class="col-lg-5">
  <input type="text" name="usertypename" class="form-control" value="<?php if(isset($UserTypeName)) { echo $UserTypeName; }?> "/>
                            </div>
                            </div>
                   </div>
                            		 <div class="form-group">
...
<?php
    $qry =mysql_query("select UserTypeId,UserTypeName from usertypemaster");
	 echo "<center>";
	  echo "<table>";
	  echo "<tr><th> UserTypeId </th><th> UserType Name </th><th></th></tr>";
	  if($qry)
	  {
	     while($rw=mysql_fetch_assoc($qry))
		 {
		 extract($rw);
		 echo "<tr><td>$UserTypeId</td><td>$UserTypeName</td>
		 <td>
		 <button class='btn btn-default btn-xs'>
		 <a href=?usertypeid=$UserTypeId&flg=Edit><i class='fa fa-pencil'></a></i></button>
	
		  <button class='btn btn-default btn-xs'>
		  <a href=?usertypeid=$UserTypeId&flg=Delete><i class='fa fa-times'></a></i></button></td></tr>";
		 }
	  }
      echo "</table>";
	  echo "</center>";
