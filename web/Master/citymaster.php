...
<!-- InstanceBeginEditable name="head" --><?php
	...
	$msg="";
	$StateId=$StateName=$CityName=$CityId="";
	 $qry= mysql_query("select max(CityId)+1 from citymaster");
								   $cityid=mysql_fetch_row($qry);
								   $CityId=$cityid[0];
	//insert code
	if(isset($_POST['insert']))
	{
	   ...
	   $qry = mysql_query("insert into citymaster values($cityid,'$cityname','$stateid');");
	   if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
///edit btn code
	if(isset($_GET["cityid"]) && $_GET['flg']=="Edit")
	{
	    $qry = mysql_query("select CityId,CityName,StateId from citymaster where cityid=".$_GET['cityid']."");
	...
	if(isset($_POST['update']))
	{
	 $cityid= $_POST['cityid'];
	   ...
	   $qry =mysql_query("update citymaster set cityname='$cityname',stateid='$stateid' where cityid=$cityid");
	if(!$qry)
	   {
	   $msg = mysql_error();
	   }
	   ...
	   {
	   $msg = "Record Updated...";
	   header('location: citymaster.php');
	   }
	}
	//delete code
	if(isset($_GET["cityid"]) && $_GET['flg']=="Delete")
	{
	    $qry = mysql_query("delete from citymaster where cityid=".$_GET['cityid']."");
	    if(!$qry)
	   {
	   $msg = mysql_error();
	   }
...
<section class="content">
	...
	<div class="row bgwhite margin">
		<ul class="breadcrumb">
								...
								<li class="logofont active">City Master</li>
      </ul>
		
		
		<center><h3 class="logofont">City Master</h3></center>
	...
	<div class="col-xs-offset-2 col-lg-8">
		...
		<div class="form-group">
			...
			<form method="post" action="">
                           ...
                           <div class="row">
                           <div class="col-lg-4">
                          	 <label class="black">Enter CityId</label>
                           </div>
                            <div class="col-lg-5">
     <input type="text" name="cityid" class="form-control" value="<?php if(isset($CityId)) { echo $CityId; }?>" />
                            </div>
                            </div>
              </div>
                                   <div class="form-group">
                           ...
                           <div class="row">
                           <div class="col-lg-4">
                          	 <label class="black">Enter City Name</label>
                           </div>
                            <div class="col-lg-5">
  <input type="text" name="cityname" class="form-control" value="<?php if(isset($CityName)) { echo $CityName; }?> "/>
                            </div>
                            </div>
              </div>
                            		<div class="form-group">
                                   ...
                                   <div class="row">
                                   <div class="col-lg-4">
                                   <label class="black">Select State</label>
                                   </div>
                                   <div class="col-lg-5">
        ...
        while($rw = mysql_fetch_row($qry))             
         ...
         ?>
                             ...
                             <div class="row">
                              ...
                              <div  class="show">
<?php
     $qry = mysql_query("select CityId,CityName,StateId from citymaster");
	 
	 echo "<center>";
	  echo "<table>";
	  echo "<tr><th> City Id </th><th> City Name </th><th>State Name</th><th></th></tr>";
	  if($qry)
	  {
	     while($rw=mysql_fetch_assoc($qry))
		 {
		 extract($rw);
		 $qry1=mysql_query("select statename from statemaster where stateid=$StateId");
		 $rw1 = mysql_fetch_row($qry1);		 
		 echo "<tr><td>$CityId</td><td>$CityName</td><td>".$rw1[0]."</td>
		 <td>
		  <button class='btn btn-default btn-xs'>
		<a href=?cityid=$CityId&flg=Edit> <i class='fa fa-pencil'></a></i></button>
	
		<button class='btn btn-default btn-xs'>
		  <a href=?cityid=$CityId&flg=Delete><i class='fa fa-times'></a></i></button></tr>";
		 }
	  }
      echo "</table>";
	  echo "</center>";
